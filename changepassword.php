...
if(isset($_POST['changepassword'])){
    ...
    $ambil = $conn->query("SELECT * FROM pelanggan WHERE id_pelanggan='$id' AND password_pelanggan='$password_lama'");
    // Mengecek akun yang cocok (email & password)
    $hasil = $ambil->num_rows;
    if (! $hasil >= 1) {
        echo "<script>alert('Password lama tidak sesuai');</script>";
        echo "<meta http-equiv='refresh' content='1;url=changepassword'>";
    }
    ...
    $query = "UPDATE pelanggan SET password_pelanggan='$password_baru' WHERE id_pelanggan='$id'";
      ...
      <div class="container">
		...
		echo $info_pelanggan['gambar_profile']; ?>" class="outer-shadow"
            ...
            <div class="row button-setting-acount">
              ...
              <div class="button-item ">
              <a  class="btn-1 active outer-shadow "><i class="fas fa-key"></i> Change Password</a>
              </div>
              </div>
          </div>
		...
		<label for="" style="margin-left:15px">Current password</label>
        <div class="input-group outer-shadow hover-in-shadow ">
        <input class="input-control" type="password"  name="password_lama">
        </div>
        <label for="" style="margin-left:15px">New Password</label>
              <div class="input-group outer-shadow hover-in-shadow ">
              <input class="input-control" type="password" name="password_baru"   class="form-control">
        </div>
         
        <label for="" style="margin-left:15px">Confirm Password</label>
              <div class="input-group outer-shadow hover-in-shadow">
              <input class="input-control" type="password" name="konf_password" class="form-control">
        </div>
              ...
              <button type="submit" name="changepassword" class="btn-1 outer-shadow hover-in-shadow">Submit</button>
            </div>
              </form>
