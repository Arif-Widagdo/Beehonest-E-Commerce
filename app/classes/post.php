...
class Post
{
    public  function addPost($data){
        ...
        $sql = "INSERT INTO `blog` (`cat_id`, `title`, `content`, `quote`, `tag`, `admin`, `status`, `image`) VALUES ('$cat_id','$title','$content', '$quote','$tag','$admin','$status','$image')";
    ...
    public function updatePost($data){
        ...
        if($_FILES['image']['name'] == NULL){
            $sql = "UPDATE `blog` SET `cat_id` = '$cat_id',`title`= '$title',`content`= '$content', `quote`= '$quote',`tag` = '$tag',`admin`= '$admin',`status`= $status WHERE id = $id";
        }else{
            $image = $_FILES['image']['name'];
            $img_ext = pathinfo($_FILES['image']['name'],PATHINFO_EXTENSION);
            $image = rand(1,888888). '.' .$img_ext;
            ...
            $sql = "UPDATE `blog` SET `cat_id` = '$cat_id',`title`= '$title',`content`= '$content', `quote`= '$quote',`tag` = '$tag',`admin`= '$admin', `image` = '$image' ,`status`= $status WHERE id = $id";
            $upload = '../uploads/' . $image;
            move_uploaded_file($_FILES['image']['tmp_name'],$upload);
